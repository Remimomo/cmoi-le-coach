...
export function buildShapeSummary(
  ...
  return {
    status,
    explanation: `${signals.join(" et ")}. ${encouragement} ${historyMessage} Pour ton objectif ${goal.toLowerCase()}, le programme privilégie ${priority.toLowerCase()} sans ajouter trop de fatigue. ${practicalExample} ${quote}`
  };
}
...
export function generateProgram(
  ...
  return selectedDays.map((day, index) => {
    const session = chooseSession(day, form, readiness, garmin, profile);
